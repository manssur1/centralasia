...
После отправки форма сохраняет заявку в базе и отправляет копию на `centralasiaenerg@gmail.com` через FormSubmit endpoint. При первой заявке FormSubmit может прислать письмо для подтверждения адреса получателя; после подтверждения следующие заявки будут приходить на эту почту автоматически.

Если письмо приходит, но база отклоняет сохранение, запусти `supabase-jwt-auth.sql` еще раз в Supabase SQL Editor. В нем есть policy `Public can create quote requests`, которая разрешает публичной форме создавать заявки.
